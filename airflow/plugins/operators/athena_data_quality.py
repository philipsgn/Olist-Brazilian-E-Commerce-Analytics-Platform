...
class AthenaDataQualityOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        # Đã sửa lại Uniqueness: Kiểm tra dòng trùng lặp hoàn toàn dựa trên Grain thực tế
        # Grain chuẩn: (order_id + item_id + source + created_at)
        dq_queries = {
            "Uniqueness": f"""
                SELECT COUNT(*) - COUNT(DISTINCT
                    CONCAT(
                        COALESCE(CAST(order_id   AS VARCHAR), ''),
                        COALESCE(CAST(item_id    AS VARCHAR), ''),
